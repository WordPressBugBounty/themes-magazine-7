...
add_action('wp_ajax_magazine_7_plugin_installer', 'magazine_7_plugin_installer'); // Install plugin
add_action('wp_ajax_magazine_7_plugin_activation', 'magazine_7_plugin_activation'); // Activate plugin
add_action('wp_ajax_magazine_7_plugin_installer_activation', 'magazine_7_plugin_installer_activation'); // Activate plugin
function magazine_7_plugin_installer()
{

  if (! current_user_can('install_plugins'))
    wp_die(__('Sorry, you are not allowed to install plugins on this site.', 'magazine-7'));
  ...
  if (! wp_verify_nonce($nonce, 'aft_installer_nonce'))
    wp_die(__('Error - unable to verify nonce, please try again.', 'magazine-7'));
...
function magazine_7_plugin_activation()
{
  if (! current_user_can('install_plugins'))
    wp_die(__('Sorry, you are not allowed to activate plugins on this site.', 'magazine-7'));
  ...
  // Check our nonce, if they don't match then bounce!
  if (! wp_verify_nonce($nonce, 'aft_installer_nonce'))
    die(__('Error - unable to verify nonce, please try again.', 'magazine-7'));
  ...
  if ($api->name) {
    $main_plugin_file = magazine_7_get_plugin_file($plugin);
    $status = 'success';
    if ($main_plugin_file) {
...
function magazine_7_plugin_installer_activation()
{
  ...
  if (!empty($list_of_plugins)) {
    $data =  magazine_7_activate_plugin_list($sanitinzed_activate);
  }
...
function magazine_7_activate_plugin_list($sanitinzed_activate)
{
  $count = 0;
